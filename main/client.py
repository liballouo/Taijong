        ...
        if game.ready:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    for i in handSprite:
                        i.cursor_on_or_not(mouseX, mouseY)
                        if i.cursorOn:
                            tileReceive = True
            ...
            drawSelfOpenTiles(win, discard_hands[0], open_hands[0])
            if not game.win:
                drawDownOpenTiles(win, discard_hands[1], open_hands[1], othersHandsLength[0])
                drawAcrossOpenTiles(win, discard_hands[2], open_hands[2], othersHandsLength[1])
                drawUpOpenTiles(win, discard_hands[3], open_hands[3], othersHandsLength[2])
            else:
                drawDownWin(win, discard_hands[1], open_hands[1], othersHands[0])
                drawAcrossWin(win, discard_hands[2], open_hands[2], othersHands[1])
                drawUpWin(win, discard_hands[3], open_hands[3], othersHands[2])
...
def drawSelfOpenTiles(win, discard_hand, open_hand):
    discardTiles = []
    x = (WIDTH*2.5)//10
    y = (HEIGHT*7.5)//10
...
#下家
def drawDownOpenTiles(win, discard_hand, open_hand, handLength):
    discardTiles = []
    x = (WIDTH*8.3)//10
    y = (HEIGHT*7.5)//10
    for i in discard_hand:
...
#對家
def drawAcrossOpenTiles(win, discard_hand, open_hand, handLength):
    discardTiles = []
    x = (WIDTH*7.5)//10
    y = (HEIGHT*2)//10
    for i in discard_hand:
...
#上家
def drawUpOpenTiles(win, discard_hand, open_hand, handLength):
    discardTiles = []
    x = (WIDTH*1.7)//10
    y = (HEIGHT*1.6)//10
    for i in discard_hand:
...
#下家(贏顯示)
def drawDownWin(win, discard_hand, open_hand, hand):
    discardTiles = []
    x = (WIDTH*8.3)//10
    y = (HEIGHT*7.5)//10
    for i in discard_hand:
...
#對家(贏顯示)
def drawAcrossWin(win, discard_hand, open_hand, hand):
    discardTiles = []
    x = (WIDTH*7.5)//10
    y = (HEIGHT*2)//10
    for i in discard_hand:
...
#上家(贏顯示)
def drawUpWin(win, discard_hand, open_hand, hand):
    discardTiles = []
    x = (WIDTH*1.7)//10
    y = (HEIGHT*1.6)//10
    for i in discard_hand:
