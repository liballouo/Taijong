    ...
    def check_other_player_move(self, tile):
        ...
        for player_num in range(len(self.players)):
            ...
            if self.players[player_num].win_move == True:
                ...
                if self.players[player_num].win_or_not == True:
                    ...
                    self.players[self.last_player].discardhand.pop()
                    self.win = True
                    return True
        ...
        for player_num in range(len(self.players)):
            ...
            # if self.players[player_num].decision_results != "":
                ...
                if decision == "kong":
                    self.players[self.last_player].discardhand.pop() # 丟的牌被槓->把她從discard hand刪掉
        ...
        for player_num in range(len(self.players)):
            if self.players[player_num].decision_types["pong"]:
                ...
                if decision == "pong":
                    self.draw = False
                    self.players[self.last_player].discardhand.pop() # 丟的牌被碰->把她從discard hand刪掉
        ...
        if self.players[(self.last_player+1)%4].decision_types["chow"]:
            ...
            if decision == "chow":
                self.draw = False
                self.players[self.last_player].discardhand.pop() # 丟的牌被吃->把她從discard hand刪掉
